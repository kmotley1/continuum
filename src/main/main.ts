...
import { createViews, placeViews, type ViewIds } from './views';
import { registerBridgeShortcut } from './shortcuts';
import { store } from './store';
import * as fs from 'fs';

let mainWindow: BrowserWindow | null;
let views: Record<ViewIds, Electron.BrowserView>;
...
function createWindow() {
  ...
  views = createViews(mainWindow);
  
  // Register global shortcuts
  registerBridgeShortcut(views);
  ...
  mainWindow.once('ready-to-show', () => {
    if (mainWindow) {
      ...
      const hasSeenOnboarding = store.get('hasSeenOnboarding', false);
  ...
  ipcMain.handle('update-layout', (_, rects: { logos: Electron.Rectangle; ai: Electron.Rectangle; }) => {
    if (mainWindow) {
      placeViews(mainWindow, rects);
    }
  });

  // Handle URL changes
  ipcMain.handle('change-url', async (_, viewId: ViewIds, url: string) => {
    try {
      console.log(`Changing ${viewId} URL to:`, url);
      await views[viewId].webContents.loadURL(url);
      store.set(`urls.${viewId}`, url);
  ...
  // Handle BrowserView setup and resize
  ipcMain.handle('setup-browser-views', () => {
    // Views are already created in createViews(), nothing to do
  });

  ipcMain.handle('resize-browser-views', () => {
    // Handled by update-layout IPC call
  });

  // Handle sermon data
  ipcMain.handle('get-sermons', () => {
    return store.get('sermons', {});
  });

  ipcMain.handle('save-sermon', (_, sermon: any) => {
    const sermons: Record<string, any> = store.get('sermons', {});
    sermons[sermon.id] = { ...sermon, updatedAt: new Date().toISOString() };
    store.set('sermons', sermons);
    return sermons;
  });

  ipcMain.handle('get-layout', () => {
    return {
      sizes: store.get('layout.sizes', [280, undefined, 420]),
      collapsed: store.get('layout.collapsed', { left: false, right: false }),
      urls: store.get('urls', { logos: 'https://app.logos.com/', ai: 'https://chat.openai.com/' }),
    };
  });

  ipcMain.handle('save-layout', (_, layout: any) => {
  ...
  ipcMain.handle('save-note', (_, note: any) => {
    const notes: Record<string, any> = store.get('notes', {});
  ...
  ipcMain.handle('set-logos-visibility', async (_, visible: boolean) => {
  ...
  ipcMain.handle('get-ai-selection', async () => {
    try {
  ...
  // Handle manual bridge trigger
        ipcMain.handle('trigger-bridge-shortcut', async (_, fromNotesEditor: boolean = false) => {
          try {
            const { clipboard } = require('electron');
            let selectedText = '';
  ...
  // Handle print to PDF
  ipcMain.handle('print-to-pdf', async (_, content: string, filePath: string) => {
    try {
